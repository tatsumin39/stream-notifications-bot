...
export async function checkAndUpdatevideo_data(data, channel_icon_url, DISCORD_WEBHOOK_URL) {
  ...
  if (rssUpdatedDate !== dbUpdatedDate) {
    ...
    if (existingData.status != apiVideoInfo.liveBroadcastContent) {
      ...
      console.log(`✅ アップデートチェック：配信予定時間が${dbscheduled_start_time}から${apischeduled_start_time}に変更されました。`);
      isChanged = true;
      ...
      if (apiVideoInfo.liveBroadcastContent === 'live') {
        ...
        console.log(`✅ アップデートチェック：ら配信予定タイトルが${existingData.title}から、${apiVideoInfo.title}に変更されました。`);
        isChanged = true;
      } 
    } else {
