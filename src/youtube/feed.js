...
export async function fetchAndStoreVideoData(DISCORD_CHANNEL_NAME, DISCORD_WEBHOOK_URL) {
  ...
  for (const { channel_id, channel_name, channel_icon_url } of channels) {
    ...
    try {
      ...
      for (const { FeedVideoId, FeedTitle, FeedPublished, FeedUpdated } of parsedEntries) {
        ...
        if (video_data && !video_data.exists) {
          ...
          const { live, scheduled_start_time, actual_start_time, duration: convertedDuration, video_id, title } = video_data;
          const data = [title, FeedPublished, FeedUpdated, video_id, channel_name, live, scheduled_start_time ? scheduled_start_time : ''];
          await checkAndUpdatevideo_data(data, channel_icon_url, DISCORD_WEBHOOK_URL);
        }
      }
    } catch (error) {
